...
export const RegisterScreen: React.FC<RegisterScreenProps> = ({ onRegister, onNavigateToLogin, isLoading, error }) => {
    ...
    <div className="min-h-[calc(100vh-250px)] flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
      <Card className="max-w-md w-full" title="">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {error && !formError && <p className="text-sm text-red-600 bg-red-50 p-3 rounded-md border border-red-200">{error}</p>}
          
          <Button type="submit" isLoading={isLoading} fullWidth size="lg" leftIcon={<SparklesIcon className="w-5 h-5"/>}>
            {isLoading ? 'Registrando...' : 'Registrar'}
          </Button>
        </form>
        <p className="mt-8 text-center text-sm text-slate-600">
          Já tem uma conta?{' '}
          <Button variant="link" onClick={onNavigateToLogin} disabled={isLoading} className="font-medium text-sky-600 hover:text-sky-700">
            Faça login
          </Button>
        </p>
      </Card>
